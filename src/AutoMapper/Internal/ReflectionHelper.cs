...
namespace AutoMapper.Internal
{
    ...
    public static class ReflectionHelper
    {
        ...
        public static MemberInfo[] GetMemberPath(Type type, string fullMemberName)
        {
            ...
            return members;
            static Type GetCurrentType(MemberInfo member)
            {
                var memberType = member.GetMemberType();
                if (memberType.IsGenericType && memberType.IsEnumerableType())
                {
        ...
        public static MemberInfo FindProperty(LambdaExpression lambdaExpression)
        {
            ...
            while (!done)
            {
                ...
                {
                    case MemberExpression { Member: var member, Expression: { NodeType: ExpressionType.Parameter or ExpressionType.Convert } } memberExpression:
                        return member;
